...
> 没有 GUI 界面的服务器上运行 qqbot 时，使用以下环境变量开启服务器模式，此时的登录二维码将通过一个内置的 web 服务器来显示，二维码获取地址为 http://服务器IP或域名:8080/qqbot/login(端口默认8080，可通过环境变量`QQBOT_SERVER_PORT`自定义端口),服务器模式默认是关闭。注意：服务器模式打开时不会在 GUI 界面中自动弹出二维码图片。
