...
QQBot 是一个用 python 实现的、基于腾讯 SmartQQ 协议的简单 QQ 机器人，可运行在 Linux 、 Windows 和 Mac OSX 平台下，所有代码均集成在一个 [qqbot.py][code] 文件中，代码量仅 500 行（不包括注释）。程序采用双线程的方式运行，且尽可能的减少了网络和登录错误（特别是所谓的 103 error ）发生的概率。
...
如果发送消息的频率过快， qq 号码可能会被锁定甚至封号。因此每发送一条消息，会强制 sleep 3~5 秒钟，每发送 10 条消息，会强制 sleep 10 秒钟。
