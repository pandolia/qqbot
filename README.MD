...
注：一般安装有 Python 的系统都已经自带 pip 了。 Windows 下，请确保安装 Python 的时候勾选了 pip 和 Add python.exe to Path。如果系统中没有 pip ，请到 [pip主页](https://pip.pypa.io/en/latest/installing/) 下载 [get-pip.py](https://bootstrap.pypa.io/get-pip.py) 并运行，可将 pip 安装至系统中。
...
以上所有命令都提供对应的 HTTP API 接口，接口的 url 地址为 http://127.0.0.1:8188/{command} ，只需要将 qq 后面的命令各参数用 "/" 分隔开替换 url 中的 command 就可以了，如: http://127.0.0.1:8188/send/buddy/jack/hello ，其他示例相见 [urltestbot.txt](https://github.com/pandolia/qqbot/blob/master/urltestbot.txt) 。注意：如果命令中含有中文或特殊字符，需要先进行 url 编码，例如： 调用  http://127.0.0.1:8188/send/buddy/jack/nihao%20%E4%BD%A0%E5%A5%BD%20wohao 将发送消息 ”nihao 你好 wohao“ 。
...
除了上面提到的 onQQMessage 响应函数，还可以注册 onNewContact/onLostContact/onInterval/onStartupComplete/onFetchComplete 五种事件的响应函数，所有事件以及函数参数格式、含义如下：
...
注意：如果采用插件的形式编写，则 @qqbotslot 是可以省略的。另外，插件里面至少要定义上述响应函数中的一个函数。
...
QQBot 启动后，会开启一个 QQBot-term 服务器监听用户通过 qq 命令行工具发过来的操作命令，此服务器端口号默认为 8188 ，可以通过修改 termServerPort 的值来修改此端口号。
...
若 cmdQrcode 项设置为 True ，则会在 term 中以文本模式显示二维码。注意：1） 文本模式在某些 terminal 中（如 windows 自带的 cmd ， gnome-terminal ， mate-terminal 等）的显示效果不太好，扫码扫不出来。2） 要使用文本模式，需要自行安装 pillow 库，可使用 pip 安装。
